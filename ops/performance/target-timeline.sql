SELECT json_build_object('orders', (SELECT coalesce(json_agg(d),'[]') FROM (
 ...
 SELECT p.id,p.order_id,p.created_at,p.status,p.scenario,m.accepted_at AS first_pg_at,m.deadline,m.result,
 extract(epoch FROM m.accepted_at-p.created_at) AS first_pg_delay_seconds
 FROM payment_attempts p LEFT JOIN mock_pg_receipts m ON m.attempt_id=p.id ORDER BY p.created_at) d));
