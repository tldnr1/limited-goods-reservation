...
param(
    ...
    [int]$Seed=20260911,[switch]$Diagnostics
)
...
function Assert-NoOtherWork {
    foreach ($name in @('goods-k6','goods-target-k6')) {
        if (Docker @('ps','--filter',"name=^/$name$",'-q')) { throw "$name 실행 중. 먼저 종료하세요." }
    }
    ...
    if ($postgres) {
        $active=Docker @('exec',$postgres,'psql','-X','-At','-U','goods','-d','postgres','-c',
...
function Assert-Ready {
    ...
    if ($up.status -ne 'success' -or @($up.data.result).Count -ne 6 -or @($up.data.result | Where-Object { $_.value[1] -ne '1' }).Count) {
...
try {
    ...
    if ($Action -eq 'Prepare') {
        ...
        # Bootstrap applies Flyway to a fresh perf DB; no fixtures or HTTP workload.
        Docker ($compose+@('up','-d','--wait','--wait-timeout','300'))
        Docker ($compose+@('stop')+$apps)
        & ./ops/reset-db.ps1 -Environment perf
        if (-not $?) { throw 'perf reset failed' }
        Docker ($compose+@('up','-d','--wait','--wait-timeout','300'))
        Write-Output 'Target perf 초기화/기동 완료. 부하는 실행하지 않았습니다. Check 후 Run을 별도로 호출하세요.'
        return
    }
    Docker ($compose+@('config','--quiet'))
    Assert-Ready
    if ($Action -eq 'Check') { Write-Output 'Target perf 역할/DB/namespace/rate/관측 준비 확인 완료 (읽기 전용).'; return }
    Docker @('image','inspect','grafana/k6:0.54.0','--format','{{.Id}}') | Out-Null
    if ([int](Sql 'SELECT count(*) FROM sales') -ne 0) { throw '이전 fixture가 있습니다. 결과 보존 후 Prepare로 초기화하세요.' }
    $runId=[Guid]::NewGuid().ToString('N')
    $directory=Join-Path $root "artifacts/performance/$(Get-Date -Format yyyyMMdd-HHmmss-fff)-target-$Scenario-$Variant"
    New-Item -ItemType Directory $directory | Out-Null
    $config=[ordered]@{runId=$runId;scenario=$Scenario;variant=$Variant;rps=$Rps;durationSeconds=$DurationSeconds;stock=$Stock;users=$Users;paymentRps=$PaymentRps;vus=$Vus;maxVus=$MaxVus;waitingRate=$WaitingRate;reservationRate=$ReservationRate;permits=$Permits;seed=$Seed}
    Save-Json $config "$directory/config.json"
    Docker ($compose+@('config')) | Set-Content "$directory/compose.yaml"
    ...
    Save-Json (Docker (@('inspect')+@($ids.Values)) | ConvertFrom-Json) "$directory/before-containers.json"
...
"@
    ...
    Save-Json @{sale=$sale;orders=$orders} "$directory/fixture.json"
    # Bounded readiness check, not a workload: projection must exist before measurement.
    $projected=$false
    for ($i=0;$i -lt 10;$i++) {
        $value=Docker @('exec',$ids.redis,'redis-cli','GET',"goods:perf:catalog:$($sale.id)")
    ...
    $network=@((Docker @('inspect',$ids.nginx) | ConvertFrom-Json))[0].NetworkSettings.Networks.PSObject.Properties.Name | Select-Object -First 1
    ...
    $arguments=@('run','--rm','--name','goods-target-k6','--network',$network,'--cpus','1.5','--memory','1g',
        ...
        '-e','TARGET_CONFIG=/results/config.json','-e','TARGET_FIXTURE=/results/fixture.json',
    ...
    if ($directory) {
        ...
        $collectors=@{
            ...
            'after-containers.json'={ Docker (@('inspect')+@($ids.Values)) };
            'redis-slowlog.txt'={ Docker @('exec',$ids.redis,'redis-cli','SLOWLOG','GET','128') };
            'services.log'={ Docker ($compose+@('logs','--no-color','--since',$(if ($loadStarted) {$loadStarted} else {[DateTimeOffset]::UtcNow.ToUnixTimeSeconds()}))) }
        }
