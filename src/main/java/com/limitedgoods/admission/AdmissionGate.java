...
public class AdmissionGate {
    ...
    public AdmissionGate(StringRedisTemplate redis,@Value("${app.admission.enabled}") boolean enabled,
        ...
        this.redis=redis; this.enabled=enabled; this.namespace=namespace; this.permits=permits; this.metrics=metrics;
    }
    public String enter() {
        if(!enabled) return null;
        String token=UUID.randomUUID().toString();
        try {
            if(!Long.valueOf(1).equals(redis.execute(ENTER,List.of(namespace+"inflight"),token,""+permits))) {
