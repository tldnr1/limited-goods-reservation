...
class PurchaseDiagnosticsTest {
    ...
    private final PurchaseService service=new PurchaseService(gate,transactions,null,false,new io.micrometer.core.instrument.simple.SimpleMeterRegistry());
    ...
    @Test void recordsFailureBeforeTransactionBodyAndPreservesException() {
        ...
        when(transactions.purchase(anyString(),anyString(),any(),any(),any())).thenThrow(failure);
    ...
    @Test void emitsAfterProxyReturnsAndIncludesCompletionPhase() {
        ...
        when(transactions.purchase(anyString(),anyString(),any(),any(),any())).thenAnswer(call->{
