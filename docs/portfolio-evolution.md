...
아래는 초기부터 완성된 설계를 따랐다는 주장이 아니라, 실험에서 발견한 문제에 따라 판단 기준을 발전시킨 기록이다. archive 수치는 해당 버전의 결과이며 현재 Java와 성능을 직접 비교하지 않는다. 새 목표·정책은 개발 전 제안으로, 현재 계약을 변경하거나 달성한 것으로 간주하지 않는다.
...
## 6. 다음 완성 단계 · 계획, 성과로 인용하지 않음

- **문제:** 순간 구매 유입과 결제 확정을 함께 보호한 반복 검증 및 변경 전후 개선 수치가 아직 없다.
- **원인·판단:** 외부 유입·점유·결제의 처리율과 시간 예산이 다르므로, 먼저 확인된 Worker 상한을 개선하고 구매 폭주가 결제 자원으로 전파되는지 검증한다.
- **해결·대안·한계:** DB 큐·현재 재고 모델을 유지하고 Worker 개선, 최소 대기실, 구매/결제 자원 격리, 접수 이후 결제 시간 정책을 적용할 계획이다. 입장권의 범위·유효시간·활성 인원·실제 점유 요청률은 개발 전 확정한다. 우선 정상 40건/초·5분 시험을 3회 통과하고 이후 50,000건 통합 시험으로 확장한다. MQ·unit별 재고는 해당 병목이 확인될 때 비교한다. 숫자로 인용할 개선율은 동일 조건의 전후 측정 후 확정한다.
